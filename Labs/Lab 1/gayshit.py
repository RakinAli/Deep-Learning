...
def preprocess_images(data, mean, std):
    # Conversion of data to float64 to perform needed calculations
...
def visualize_images(images, labels, label_names, number=5):
    ...
    for i in range(number):
        for j in range(number):
            ...
            axes[i, j].imshow(images[indices[i * number + j]],
...
def visualize_weight(weight, label_names):
    ...
    for i in range(axes.shape[0]):
        for j in range(axes.shape[1]):
            ...
            axes[i, j].imshow(images[i * axes.shape[1] + j],
                              interpolation='bicubic')
    plt.show()
...
def main():
    ...
    # Data preprocessing
    data_train, mean_train, std_train = preprocess_images(
        data_train, mean=None, std=None)  # Preprocess traning data
    data_train = data_train.T  # Transpose data to get the appropriate format --> d x n
    data_val = preprocess_images(data_val, mean_train, std_train)[
        0].T  # Std. val. using training mean and std
    data_test = preprocess_images(data_test, mean_train, std_train)[
        0].T  # Std. test using training mean and std
    # Convert training labels to one-hot matrix
    labels_train = one_hot(labels_train, len(label_names))
    # Convert validation labels to one-hot matrix
    labels_val = one_hot(labels_val, len(label_names))
    # Convert test labels to one-hot matrix
    labels_test = one_hot(labels_test, len(label_names))

    # Initialize model parameters
    weight = np.random.normal(
        0, 0.01, (len(label_names), data_train.shape[0]))  # Dim: k x d
    ...
    for _ in tqdm(range(n_epoch)):
        for j in range(int(data_train.shape[1] / n_batch)):
            ...
            weight -= eta * delta_w
            bias -= eta * delta_b
        training_loss.append(compute_loss(
            data_train, labels_train, weight, bias))
        validation_loss.append(compute_loss(
            data_val, labels_val, weight, bias))
        training_cost.append(compute_cost(
            data_train, labels_train, weight, bias, lmb))
        validation_cost.append(compute_cost(
            data_val, labels_val, weight, bias, lmb))
    ...
    print("Accuracy on test data: " +
          str(compute_accuracy(data_test, labels_test, weight, bias) * 100) + "%")
