...
def mini_batch_training(data_train, label_train, data_val, label_val, data_test, label_test, weight, bias, lmbd, eta, n_batch, n_epochs):
  ...
  for i in range(n_epochs):
    for j in range(0, int(data_train.shape[1] / n_batch)):
      # Getting the mini-batch
      data_batch = data_train[:, j * n_batch : (j + 1) * n_batch]
      label_batch = label_train[j * n_batch : (j + 1) * n_batch]

      # Getting the probabilities
      probabilities = evaluate_classifier(data_batch, weight, bias)

      # Computing the gradients
      gradient_weight, gradient_bias = ComputeGradients(data_batch, label_batch, probabilities, weight, lmbd)

      # Updating the weights and biases
      weight = weight - eta * gradient_weight
      bias = bias - eta * gradient_bias
    ...
    cost_train.append(compute_Cost(data_train, label_train, weight, bias, lmbd))
    cost_val.append(compute_Cost(data_val, label_val, weight, bias, lmbd))

  return weight, bias, loss_train, loss_val, cost_train, cost_val
